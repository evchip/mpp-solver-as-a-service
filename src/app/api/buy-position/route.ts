  ...
  if (!body.order_id) {
    return payment.withReceipt(
      Response.json(
        {
          error:
            "order_id required. Deposit into escrow first, then call with your order_id.",
        },
        { status: 400 },
      ),
    )
...
async function handleDirectOrder(body: BuyPositionRequest) {
  ...
  if (!token_id || !amount_usd || !recipient_polygon) {
    return Response.json(
      { error: "token_id, amount_usd, recipient_polygon required" },
      { status: 400 },
    )
  }
  ...
  if (balance === 0n) {
    return Response.json(
      { error: "Order matched but no shares settled yet", fill },
      { status: 502 },
    )
  }
...
async function handleEscrowOrder(body: BuyPositionRequest) {
  ...
  if (!order_id || !recipient_polygon) {
    return Response.json(
      { error: "order_id and recipient_polygon required" },
      { status: 400 },
    )
  }
  ...
  if (user === "0x0000000000000000000000000000000000000000") {
    return Response.json(
      { error: "Order not found in escrow" },
      { status: 404 },
    )
  }
  if (settled) {
  ...
  const expectedHash = keccak256(
    encodePacked(["address"], [recipient_polygon as `0x${string}`]),
  )
  if (expectedHash !== recipientHash) {
    return Response.json(
      { error: "Recipient does not match escrow order" },
      { status: 400 },
    )
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (balance > 0n) break
    console.log(
      `[escrow] Settlement check ${i + 1}/5: no shares yet, waiting...`,
    )
  }
  if (balance === 0n) {
    return Response.json(
  ...
  await polygonClient.waitForTransactionReceipt({
    hash: txHash,
    timeout: 60_000,
  })
