...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return payment.withReceipt(
      Response.json(
        { error: `Market search failed: ${err.message}` },
        { status: 502 },
      ),
    )
  }
...
Respond with ONLY valid JSON, no markdown:
{
  ...
  const usdc =
    process.env.USDC_TEMPO ?? "0x20c000000000000000000000b9537d11c60e8b50"
  ...
  return payment.withReceipt(
    Response.json({
      ...
      next_steps: [
        ...
        `export RECIPIENT=<REPLACE_WITH_YOUR_POLYGON_ADDRESS>`,
        ...
        `# 4. Call solver (pays $0.50 service fee via MPP, solver fills + proves + settles)`,
